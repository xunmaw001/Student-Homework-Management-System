                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 级联表的字段 -->
                                    <div class="form-group col-md-6">
                                        <label>班级课程老师关系</label>
                                        <div>
                                            <select id="banjiKechengLaoshiSelect" name="banjiKechengLaoshiSelect"
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="banjiKechengLaoshiId" name="banjiKechengLaoshiId" type="hidden">
                                    <div class="form-group col-md-6">
                                        <label>作业名字</label>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            let data = {};
            getContent();
           if($("#banjiKechengLaoshiId") !=null){
               var banjiKechengLaoshiId = $("#banjiKechengLaoshiId").val();
               if(banjiKechengLaoshiId == null || banjiKechengLaoshiId =='' || banjiKechengLaoshiId == 'null'){
                   alert("班级课程不能为空");
                   return;
               }
           }
    ...
    // 查询列表
        <!-- 查询当前表的所有列表 -->
        <!-- 查询级联表的所有列表 -->
        function banjiKechengLaoshiSelect() {
            //填充下拉框选项
            http("banjiKechengLaoshi/page?page=1&limit=100&sort=&order=", "GET", {}, (res) => {
                if(res.code == 0){
                    banjiKechengLaoshiOptions = res.data.list;
                }
            });
        }

        function banjiKechengLaoshiSelectOne(id) {
            http("banjiKechengLaoshi/info/"+id, "GET", {}, (res) => {
                if(res.code == 0){
                ruleForm = res.data;
                banjiKechengLaoshiShowImg();
                banjiKechengLaoshiDataBind();
            }
        });
        }
    ...
    <!-- 初始化级联表的下拉框(要根据内容修改) -->
    <!-- 初始化级联表的下拉框(要根据内容修改) -->
        function initializationbanjiKechengLaoshiSelect() {
            var banjiKechengLaoshiSelect = document.getElementById('banjiKechengLaoshiSelect');
            if(banjiKechengLaoshiSelect != null && banjiKechengLaoshiOptions != null  && banjiKechengLaoshiOptions.length > 0 ) {
                for (var i = 0; i < banjiKechengLaoshiOptions.length; i++) {
                        banjiKechengLaoshiSelect.add(new Option("班级名:"+banjiKechengLaoshiOptions[i].banjiName+" 课程名:"+banjiKechengLaoshiOptions[i].kechengName+" 老师名:"+banjiKechengLaoshiOptions[i].laoshiName, banjiKechengLaoshiOptions[i].id));
                }

                $("#banjiKechengLaoshiSelect").change(function(e) {
                        banjiKechengLaoshiSelectOne(e.target.value);
                });
            }
    ...
    function setSelectOption() {
        ...
        <!--  级联表的下拉框回显  -->
            ...
            if(banjiKechengLaoshiSelect != null && banjiKechengLaoshiOptions != null  && banjiKechengLaoshiOptions.length > 0 ) {
                for (var i = 0; i < banjiKechengLaoshiOptions.length; i++) {
                    if (banjiKechengLaoshiOptions[i].id == ruleForm.banjiKechengLaoshiId) {//下拉框value对比,如果一致就赋值汉字
                        banjiKechengLaoshiSelect.options[i+1].selected = true;
                        $("#banjiKechengLaoshiSelect" ).selectpicker('refresh');
                    }
                }
            }
    }
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            ...
            messages: {
                banjiKechengLaoshiId: "班级课程不能为空",
                zuoyeName: "作业名字不能为空",
                zuoyeFile: "布置的作业不能为空",
                zuoyeContent: "作业详情不能为空",
                insertTime: "发布作业时间不能为空",
                endTime: "作业截止时间不能为空",
            }
        }).form();
    }
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#banjiKechengLaoshiId").val(ruleForm.banjiKechengLaoshiId);
    ...
    <!--  级联表的数据回显  -->
    function banjiKechengLaoshiDataBind(){

                    <!-- 把id赋值给当前表的id-->
        $("#banjiKechengLaoshiId").val(ruleForm.id);
    ...
    function banjiKechengLaoshiShowImg() {
    }
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            <!--  初始化级联表的下拉框  -->
            initializationbanjiKechengLaoshiSelect();
